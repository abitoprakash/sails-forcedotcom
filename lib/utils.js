...
/**
 * This function will return the select fields for the query
 * @param {Array<String>} querySelect - Fields to select
 * @param {Object} collection - Waterline collection
 * @returns {Array<String>} select fields
 */
function getSelectFields(querySelect, collection) {
  if (querySelect && querySelect.length) {
    return querySelect;
  }

  if (collection && collection.definition && _.isObject(collection.definition)) {
    return Object.values(collection.definition).reduce((acc, attr) => {
      // Only add to select fields if columnName is defined
      if (attr.columnName) {
        acc.push(attr.columnName);
      }
