...
module.exports = (function () {
  ...
  function parseClause(original) {
    ...
    /**
   * parseDatabyDatatype - Method which parses the Data with respect to Model attribute data types 
  ...
  */
  function parseDataByDatatype(data, collectionDef) {
    ...
    const dateTimeColumns = Object.entries(attributes).reduce((acc, [key, value]) => {
      const columnType = (value.autoMigrations && value.autoMigrations.columnType) || value.columnType || value.type;
          ...
          if (dateTimeColumns[key]) {
            ...
            if (_.isPlainObject(item)) {
              return _.mapValues(item, (value) => {
                return dateTimeColumns[key](value);
              });
            }
          }
