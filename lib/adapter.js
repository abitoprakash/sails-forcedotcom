...
module.exports = (function () {
  ...
  var adapter = {
    ...
    count: function(connectionName, options, cb) {
      var collectionName = options.using
      ...
      whereClause = parseDataByDatatype(whereClause, collection);
  ...
  // This will convert and, or, like, in, >=, >, <=, < into $and, $or, $like, $in, $gte, $gt, $lte, $lt
  // Since the new format of sails v1 is not compatible with jsforce
  function parseClause(original) {
    ...
    const mapperFn = (_value, key) => operatorMap[key] || key;
    ...
    /**
   * This function will parse the data based on the datatype of the attributes
   * and convert the data to the respective datatype before sending to Salesforce
   *  (else all the data by default is converted to string)
   *  Reference : https://jsforce.github.io/jsforce/doc/SfDate.html
   * @param {object} data 
   * @param {object} collectionDef 
   * @returns {object}
  */
  function parseDataByDatatype(data, collectionDef) {
    const attributes = collectionDef && _.isPlainObject(collectionDef.definition) ? collectionDef.definition : {};
 
    const dateTimeColumns = Object.entries(attributes).reduce((acc, [key, value]) => {
      const columnType = _.get(value, 'autoMigrations.columnType', value.columnType || value.type)
      const field = value.columnName || key;

      // Add date type formatters here
    ...
    const parseQuery = (obj) => {
      if(_.isPlainObject(obj)) {
        return _.mapValues(obj, (item, key) => {
          if (dateTimeColumns[key]) {
            if (typeof item === 'string') {
              return dateTimeColumns[key](item);
            }

            if (_.isPlainObject(item)) {
              return _.mapValues(item, (value) => dateTimeColumns[key](value));
            }
          }
      ...
      if(_.isArray(obj)) {
          return obj.map((arrItem) => parseQuery(arrItem))
      }
      return obj
    }
