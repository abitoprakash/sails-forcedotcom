  ...
  */
  function parseDataByDatatype(data, collectionDef) {
    ...
    const dateTimeColumns = Object.entries(attributes).reduce((acc, [key, value]) => {
        if (value.columnType === 'datetime' || value.columnType === 'date') {
          acc.push(value.columnName || key);
        }
        return acc
      }, []);
    ...
    const parseQuery = (obj) => {
      if(_.isPlainObject(obj)) {
        return _.mapValues(newObj, (item, key) => {
          if (dateTimeColumns.includes(key)) {
            if (typeof item === 'string') {
              return jsforce.SfDate.toDateTimeLiteral(item);
            }

            if (_.isPlainObject(item)) {
              return _.mapValues(item, (value) => {
                return jsforce.SfDate.toDateTimeLiteral(value);
              });
            }
          }
